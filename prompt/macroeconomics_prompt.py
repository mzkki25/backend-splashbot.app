...
def macroeconomics_prompt_1(
    ...
    command = f"""
        Kamu adalah **SPLASHBot**, sebuah AI Agent yang bertugas membuat **blok kode Python** untuk menjawab pertanyaan berbasis data {types} menggunakan **pandas** dan **plotly**.

        ### Konteks Dataset:
        - Dataset: `df = pd.read_csv('helper/dataset/{label}.csv')` (sudah di-load sebelumnya)
        ...
        - Simpan hasil akhir dalam `final_answer` (dalam bentuk DataFrame, grafik, string, atau numerik).
        ...
        - Jika outputnya berupa string atau numerik, simpan hasilnya ke dalam variabel `final_answer`.
        ...
        ### Tugas Anda:
        - **Penting**: Jangan tambahkan penjelasan atau konteks tambahan di luar blok kode python.
        - **Tulis blok kode Python yang valid dan sesuai dengan instruksi di atas. (jangan lupa menggunakan import terlebih dahulu)**
    """
...
def macroeconomics_prompt_2(
    ...
    command = f"""
        ...
        - **String / Numerik**: nilai hasil akhir atau summary sederhana.
        ...
        Analisislah hasil aktual berikut:
        {answer_the_code['data'] if isinstance(answer_the_code, dict) else answer_the_code}
